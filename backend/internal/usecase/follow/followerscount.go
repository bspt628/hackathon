...
func (fc *FollowUsecase) UpdateAndGetFollowersCount(ctx context.Context, userID string) (int32, error) {
	if fc == nil || fc.followDAO == nil {
		return 0, errors.New("UserUseCase or UserDAO is nil")
	}

	// フォロー情報を保存
	count, err := fc.followDAO.UpdateAndGetFollowersCount(ctx, userID)
	if err != nil {
